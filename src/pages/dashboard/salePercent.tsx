...
const SalePercent: FC<{ loading: boolean }> = ({ loading }) => {
  ...
  return (
    <Card
      ...
      title={t({ id: 'app.dashboard.salePercent.proportionOfSales' })}
      loading={loading}
      extra={
        <Radio.Group value={dataType} onChange={e => setDataType(e.target.value)} buttonStyle="solid">
          <Radio.Button value="all">{t({ id: 'app.dashboard.salePercent.all' })}</Radio.Button>
          <Radio.Button value="online">{t({ id: 'app.dashboard.salePercent.online' })}</Radio.Button>
          <Radio.Button value="offline">{t({ id: 'app.dashboard.salePercent.offline' })}</Radio.Button>
        </Radio.Group>
      }
    >
      <Row gutter={20}>
