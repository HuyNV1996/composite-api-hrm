...
const HeaderNoticeComponent: FC = () => {
  ...
  const getNotice = async () => {
    setLoading(true);
    const { status, result } = (await getNoticeList()) as any;
    ...
    <div>
      <Spin tip="Loading..." indicator={antIcon} spinning={loading}>
        <Tabs defaultActiveKey="1">
          <TabPane
            tab={`Mới(${noticeListFilter('notification').length})`}
            key="1">
            <List
          ...
          <TabPane
            tab={`Tin nhắn(${noticeListFilter('message').length})`}
            key="2">
            <List
          ...
          <TabPane tab={`Tin(${noticeListFilter('event').length})`} key="3">
            <List
